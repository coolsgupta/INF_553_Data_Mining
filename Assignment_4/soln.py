...
pairwise_users = businesswise_users.flatMap(lambda x: get_user_pairs(x)).reduceByKey(lambda x, y: x + y).filter(
    lambda x: len(x[1]) >= filter_threshold).map(lambda x: x[0])
...
betweenness_collection = betweenness_rdd.collect()

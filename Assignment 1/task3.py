...
if __name__ == '__main__':
    ...
    reviews_json = sc.textFile(argv[1]).map(lambda x: json.loads(x)).map(lambda x: (x['business_id'], 1))
    ...
    if argv[3] == 'default':
        ...
        reviews_data = reviews_json.partitionBy(argv[4], hash_partition_business_id)

    # business_review_map = reviews_data.map(lambda x: (x[0], 1))
    business_review_count = reviews_data.reduceByKey(lambda a, b: a + b)

    results['n_partitions'] = reviews_data.getNumPartitions()
    results['n_items'] = reviews_data.glom().map(len).collect()
